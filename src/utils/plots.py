...
def plot_prices_and_returns(datasets, figsize=(14, 10), recent_days=None):
    ...
    # Applying style for better readability
    ...
    for i, (asset_name, df) in enumerate(datasets.items()):
        ...
        # Plotting prices ON TOP ROW - using 'price' column (lowercase)
        ax_price = ax[0, i]
        ...
        ax_returns.axhline(y=mean_return, color='red', linestyle='--', linewidth=1.5, 
                          label=f"Mean: {mean_return:.6f}")
        
        # Adding zero line for reference
        ax_returns.axhline(y=0, color='black', linestyle='-', linewidth=1.5, alpha=0.3)
        
        # Formatting log returns plot
